(function () {
    ...
    async function loadCadsus() {
        ...
        area.innerHTML = '<div style="color:#1a56db;font-size:13px;padding:8px 0">Consultando CADSUS...</div>';

        const cpfFmt = formatCPF(state.cpf); // "000.000.000-00"

        let found = null;
        let fonte  = '';

        // ── 1) Primário: v3apiguaruja (GET com cookies de sessão) ──────────────
        try {
            const url = 'https://v3apiguaruja.saudesimples.net/municipes/cpf_duplicado'
                      + '?cpf=' + encodeURIComponent(cpfFmt) + '&id=';
            const result = await gmGetV3(url);
            if (result && result.length) {
                found = normalizeV3(result[0]);
                fonte = 'v3';
            }
        } catch (err) {
            console.warn('[CNES] v3apiguaruja falhou:', err.message, '— tentando wscadsus...');
        }

        // ── 2) Fallback: wscadsus (POST) ────────────────────────────────────────
        if (!found) {
            try {
                ...
                console.warn('[CNES] wscadsus falhou:', err.message);
            }
        }

        console.log('[CNES] CADSUS fonte:', fonte || 'nenhuma', '| found:', found);

        // ── Popula estado editável ───────────────────────────────────────────────
        state.editCep        = found ? (found.residencia_cep    || '') : '';
        state.editNumero     = found ? (found.residencia_numero  || '') : '';
        state.editNascimento = found ? formatDate(found.data_nascimento) : '';
        state.editNomeMae    = found ? (found.nome_mae           || '') : '';
        state.editNomePai    = found ? (found.nome_pai           || '') : '';
        state.editTelefone   = found ? normalizeTelefone(found.telefone_celular) : '';
